%% Problem 1
% Modified by: Morgan Yost
% Math 502 Take Home Midterm 1
%%
% bvp_2.m 
...
A = (3 + 4*exp(-2) - sin(1) + cos(1))/(exp(-3)-exp(-2));
B = -4-A;
f = @(x) 9*A*exp(-3*x)+4*B*exp(-2*x)-sin(x) + cos(x);  % right hand side function
utrue = @(x) A*exp(-3*x) + B*exp(-2*x) + sin(x) - cos(x);  % true soln
